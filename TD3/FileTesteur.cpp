...
TEST(FileTestAvecComparable, FileDeComparableNo1) {
	File<Comparable> comparable;
	comparable.enfiler(c1);
	ASSERT_EQ(c1, comparable.defiler());

}

TEST(FileTestAvecComparable, FileDeComparable2Elements) {
	File<Comparable> comparable;
	comparable.enfiler(c1);
	comparable.enfiler(c2);
	ASSERT_EQ(2, comparable.taille());

}

TEST(FileTestAvecComparable, FileDeComparableCopieAvecConstructeur) {
	File<Comparable> comparable;
	comparable.enfiler(c1);
	comparable.enfiler(c2);

	File<Comparable> comparableCopie(comparable);

	ASSERT_EQ( comparable.taille(),comparableCopie.taille());

}

TEST(FileTestAvecComparable, FileDeComparableCopieAvecOperateurEgal) {
	File<Comparable> comparable;
	comparable.enfiler(c1);
	comparable.enfiler(c2);

	File<Comparable> comparableCopie;

	comparableCopie=comparable;

	ASSERT_EQ( c1,comparableCopie[0]);
	ASSERT_EQ( c2,comparableCopie[1]);
...
TEST_F(FileTest, EnfileUnFloatOK) {
	file3.enfiler(val4);
	EXPECT_EQ(val4, file3.premier());
	EXPECT_FALSE(file3.estVide());
	EXPECT_EQ(1, file3.taille());
	EXPECT_EQ(val4, file3.defiler());
	EXPECT_TRUE(file3.estVide());
	EXPECT_EQ(0, file3.taille());
}
